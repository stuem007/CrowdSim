...
class Boid {
  ...
  float MAX_SEE_AHEAD = 1.7;
  float MAX_AVOID_FORCE = .2;
  ...
  private PVector collisionAvoidance(){
    PVector tempVel = new PVector(velocity.x, velocity.y);
    ahead = PVector.add(location, PVector.mult(tempVel.normalize(), MAX_SEE_AHEAD));
    tempVel = new PVector(velocity.x, velocity.y);
    ahead2 = PVector.add(location, PVector.mult(tempVel.normalize(), MAX_SEE_AHEAD*2)); // calculate the ahead2 vector
  ...
  private Obstacle findMostThreateningObstacle() {
      ...
      for (int i = 0; i < Obstacles.size(); i++) {
          ...
          Boolean collision = (PVector.sub(obstacle.location, ahead).mag() <= obstacle.r) || (PVector.sub(obstacle.location, ahead).mag() <= obstacle.r);
   
          // "position" is the character's current position
          if (collision && (mostThreatening == null || PVector.sub(location, obstacle.location).mag() < PVector.sub(location, mostThreatening.location).mag())) {
